...
describe("CardSet test", () => {
    ...
    it("should contain 13 cards of each colour", () => {
        let clubs: Card[] = cardSet.cards.filter((card) => {
            return card.colour === "CLUBS"
        });
        let diamonds: Card[] = cardSet.cards.filter((card) => {
            return card.colour === "DIAMONDS"
        });
        let spades: Card[] = cardSet.cards.filter((card) => {
            return card.colour === "SPADES"
        });
        let hearts: Card[] = cardSet.cards.filter((card) => {
            return card.colour === "HEARTS"
        });
    ...
    it("should contain 4 cards of each rank", () => {
        ...
        for (let rank in RankValues) {
            rankSet = cardSet.cards.filter((card) => {
    ...
    it("should draw card and remove it from card set", () => {
        ...
        let isCardInSet: Card | undefined = _.find(cardSet.cards, (card) => {
